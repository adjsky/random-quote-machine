...
function changeQuote(e) {
    ...
    tweetQuote.href = `https://twitter.com/intent/tweet?hashtags=ulalala&text="${quotes[randomNumber].text}"`
    content.style.opacity = 0;
    setTimeout(() => {
        body.style.background = `${quotes[randomNumber].color}`;
        buttons.forEach(button => button.style.background = quotes[randomNumber].color);
    }, 500);
}
